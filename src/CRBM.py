...
class CRBM(object):
  ...
  def __init__(self, name, fully_connected = True, v_length = 1, v_channels = 784, f_length = 1,  f_number = 400,
    ...
    try:
      ...
      with tf.variable_scope(name):
         with tf.device('/cpu:0'):
          self.kernels            = tf.get_variable('weights', (f_length,v_channels,f_number), initializer=tf.truncated_normal_initializer(stddev=init_weight_stddev, dtype=tf.float32), dtype=tf.float32)
          ...
          self.vitesse_kernels    = tf.get_variable('vitesse_weights', (f_length, v_channels,f_number), initializer=tf.constant_initializer(0), dtype=tf.float32)
          ...
          if gaussian_unit:
            self.sigma              = tf.get_variable('sigma', (v_length*batch_size*v_channels), initializer=tf.constant_initializer(self.gaussian_variance * self.gaussian_variance), dtype = tf.float32)
  ...
  def compute_energy(self, visible, hidden, method = 'forward'):
    ...
    if method == 'forward':
      ...
      if self.padding:
        ...
        conv = tf.nn.conv1d(self._get_padded_hidden(hidden), self._get_flipped_kernel(), stride=1, padding='VALID')
      operand = visible
  ...
  def infer_probability(self, operand, method, result = 'hidden'):
    ...
    if method == 'forward':
      ...
      if self.prob_maxpooling: 
        ...
        sum = tf.nn.depthwise_conv1d(exp, custom_kernel, [1, 2, 1], padding='VALID')
        sum = tf.add(1.0,sum)
