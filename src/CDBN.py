...
class CDBN(object):
    ...
    def add_softmax_layer(self, output_classes, learning_rate, fine_tune=False):
        ...
        try:
            if self.locked:
                raise ValueError(
                    'Trying to add softmax layer to the CDBN ' + self.name + ' which has already been locked')
            if self.softmax_layer:
                    ...
                    with tf.device('/cpu:0'):
                        self.W = tf.get_variable('weights_softmax', (self.output, output_classes),
                                                 initializer=tf.truncated_normal_initializer(stddev=1 / self.output,
                                                                                             dtype=tf.float32),
                                                 dtype=tf.float32)
                        self.b = tf.get_variable('bias_softmax', (output_classes),
                ...
                if fine_tune:
                    ...
                    (ret_w_0, ret_w_1), ret_b = self.softmax_trainer.compute_gradients(cross_entropy,
    ...
    def do_eval(self, f1=False):
        ...
        if f1:
            ...
            true_positive = tf.math.reduce_sum(
                tf.cast(tf.logical_and(tf.equal(predicted_class, ones), tf.equal(real_class, ones)), tf.float32))
            tp_count = 0
            false_positive = tf.math.reduce_sum(
                tf.cast(tf.logical_and(tf.equal(predicted_class, ones), tf.equal(real_class, zeros)), tf.float32))
            fp_count = 0
            true_negative = tf.math.reduce_sum(
                tf.cast(tf.logical_and(tf.equal(predicted_class, zeros), tf.equal(real_class, zeros)), tf.float32))
            tn_count = 0
            false_negative = tf.math.reduce_sum(
                tf.cast(tf.logical_and(tf.equal(predicted_class, zeros), tf.equal(real_class, ones)), tf.float32))
            fn_count = 0
        else:
        ...
        for i in range(2):
            ...
            for step in range(steps_per_epoch):
                ...
                if f1:
                    a, b, c, d = self.session.run([true_positive, false_positive, true_negative, false_negative],
                                                  feed_dict={input_placeholder: visible, y_: labels_feed})
                    tp_count += a
                    fp_count += b
                    tn_count += c
                    fn_count += d
                else:
                    true_count += self.session.run(correct_count,
                                                   feed_dict={input_placeholder: visible, y_: labels_feed})
            ...
            if f1:
                ...
                print('Successfully evaluated the CDBN on the', test_type[i],
                      'set: \n Precision is %0.02f percent \n Recall is %0.02f percent \n F1 score is %0.02f\n tp: %d ---  fp: %d ---  tn: %d ---  fn: %d\n Overall precision is %0.02f percent' % (
                      precision * 100, recall * 100, f1_score, tp_count, fp_count, tn_count, fn_count,
                      overall_precision * 100))
            else:
                precision = true_count / num_examples
                print('Successfully evaluated the CDBN on the', test_type[i],
                      'set: \n %d examples are correctly classified out of %d total examples\n Accuracy is %0.02f percent' % (
                      true_count, num_examples, precision * 100))
    ...
    def _pretrain_layer(self, rbm_layer_name, number_step, n=1):
        ...
        for i in range(1, number_step):
            ...
            if self.verbosity == 1 and i % 500 == 0 and not (i % 1000 == 0):
                print(
                    'Step %d: reconstruction error = %.05f (%.3f sec)  -----  Estimated remaining time is %.0f sec' % (
            ...
            if self.verbosity == 2 and i % 100 == 0 and not (i % 1000 == 0):
                print(
                    'Step %d: reconstruction error = %.05f (%.3f sec), err = %.05f, con = %.05f and weight upgrade to weight ratio is %.2f percent  -----  Estimated remaining time is %.0f sec' % (
                ...
                print(
                    'Step %d: reconstruction error = %.05f (%.3f sec) and weight upgrade to weight ratio is %.2f percent  -----  Estimated remaining time is %.0f sec' % (
    ...
    def _do_softmax_training(self, step=5000, fine_tune=False, learning_rate=0.5):
        ...
        for i in range(1, step):
            ...
            visible = np.reshape(images_feed, self.input)
            if fine_tune:
                _, a = self.session.run([self.train_step, self.cross_entropy_mean],
                                        feed_dict={self.input_placeholder: visible, self.y_: labels_feed})
            else:
                _, a, b = self.session.run([self.train_step, self.cross_entropy_mean, self.control],
                                           feed_dict={self.input_placeholder: visible, self.y_: labels_feed})
                average_control += b
            average_cost += a
            if self.verbosity > 0 and i % 250 == 0:
                print(
                    'Step %d: cost is %.7f----- control value (gradient rate) : %.7f percent --- Estimated remaining time is %.0f sec' % (
                    i, average_cost / 250, average_control / 250 * 100, (step - i) * (time.time() - start) / i))
                average_cost = 0
                average_control = 0
        if self.verbosity > 0:
            print('--------------------------')
